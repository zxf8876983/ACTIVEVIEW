...
def test_allow_test_flag_alone_cannot_unlock(tmp_path):
    with pytest.raises(TestGateError):
        assert_test_allowed({"experiment": {"experiment_id": "EXP001", "status": "COMPLETED", "decision": "ACCEPT"}, "protocol": {}}, allow_test=True)
...
def test_non_accepted_or_unauthorized_manifest_fails_closed(tmp_path):
    manifest, auth, config = _final_manifest(tmp_path)
    manifest["experiment"]["decision"] = "REJECT"
