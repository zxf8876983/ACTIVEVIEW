...
def _write_report(path: Path, summary: Mapping[str, Any], references: Sequence[Mapping[str, Any]], paths: Mapping[str, str]) -> None:
    ...
    high_actions = ", ".join(f"{name} ({rate:.2%})" for name, rate in sorted(summary["high_regret_action_rates"].items(), key=lambda item: -item[1])[:5]) or "none"
    high_regions = ", ".join(f"{name} ({rate:.2%})" for name, rate in sorted(summary["high_regret_region_rates"].items(), key=lambda item: -item[1])) or "none"
    ...
    lines += ["", "The dominant failure type is **" + max(labels, key=lambda key: taxonomy[key]["count"]) + "**. High-regret action rates are: " + high_actions + ". Region high-regret rates are: " + high_regions + ".", "", "## Candidate miss versus utility quality", "", f"- Candidate exact miss rate: {summary['candidate_miss']['miss_ratio']:.2%} ({summary['candidate_miss']['miss_count']} episodes).", f"- For misses, CandidateOracle utility minus selected utility: mean {summary['candidate_miss']['absolute_utility_gap']['mean']:.4f}, median {summary['candidate_miss']['absolute_utility_gap']['median']:.4f}, p90 {summary['candidate_miss']['absolute_utility_gap']['p90']:.4f}.", f"- Among misses with CandidateOracle utility >1e-6, selected utility reaches ≥90%/75%/50% of CandidateOracle in {summary['candidate_miss']['ratio_thresholds']['selected_at_least_90pct_oracle']:.2%}/{summary['candidate_miss']['ratio_thresholds']['selected_at_least_75pct_oracle']:.2%}/{summary['candidate_miss']['ratio_thresholds']['selected_at_least_50pct_oracle']:.2%}.", "This directly tests whether low exact hit is mainly near-equivalent selection or materially bad utility loss.", "", "## Action-class and region findings", "", "Lowest Set Accuracy classes: " + ", ".join(f"{name} ({value['Set_accuracy']:.2%})" for name, value in hardest) + ".", "Highest mean-regret classes: " + ", ".join(f"{name} ({value['regret']['mean']:.3f})" for name, value in highest_regret) + ".", "Region breakdown is in the machine-readable summary; it should not be treated as a scene-held-out result.", "", "## Current state and geometry diagnostics", "", "- Spearman correlations with regret (absolute ordering): " + ", ".join(f"{name}={value:+.3f}" for name, value in strongest) + ".", "- State-group summaries compare entropy, margin, pose confidence, current correctness, move rate and SafeOracle headroom in the JSON artifact.", "- Geometry separates CandidateOracle geometry (all Episodes) from SafeOracle move geometry (Move-only Episodes); selected geometry and candidate-set utility-gap bins are also reported.", "- Symmetric-geometry analysis uses explicit tolerances and a data-derived q90 utility-difference threshold; it is indirect evidence only and does not estimate body yaw.", "", "## Record-level concentration", "", f"- Top 10% records account for {summary['record_level']['catastrophic_episode_share_in_top10pct_records']:.2%} of catastrophic top-5% episodes ({summary['record_level']['catastrophic_concentration_ratio_vs_uniform']:.2f}x the uniform 10% baseline).", "- Worst and best 20 motion records are included in `record_failure_table.csv` and the JSON summary.", "", "## Scientific answers", "", f"1. Catastrophic failures are substantially but not exclusively concentrated: the worst 10% of records account for {summary['record_level']['catastrophic_episode_share_in_top10pct_records']:.2%} of top-5% failures.", f"2. The dominant decision failure is {max(labels, key=lambda key: taxonomy[key]['count'])}; wrong-candidate severity is separated into C1/C2.", f"3. Exact candidate hit versus aggregate headroom is explained by the miss utility-gap and ratio statistics above.", f"4. The hardest action classes by Set accuracy are {', '.join(name for name, _ in hardest)}.", f"5. The strongest state correlation by absolute Spearman magnitude is {strongest[0][0]} ({strongest[0][1]:+.3f}); this is descriptive, not causal.", "6. Geometry and azimuth results are descriptive binned evidence; no single geometry variable is assumed causal.", f"7. Symmetric-geometry ambiguity affects {summary['symmetric_geometry_ambiguity']['episode_count']} episodes under the stated tolerance, with {summary['symmetric_geometry_ambiguity']['large_difference_pair_count']} large-difference pairs; explicit enrichment ratio={summary['symmetric_geometry_ambiguity']['enrichment_ratio']:.2f}x versus the overall high-regret baseline.", "8. Evidence for adding perceived body orientation is **weak / inconclusive** unless symmetric ambiguity is clearly enriched among high-regret cases; no body yaw was added here.", "9. Current evidence most directly supports hard-example/long-tail analysis (E) and improved current-state/candidate-set representation (B/A) as hypotheses for later review. It does not justify changing Stage C-v0 in this task.", "", "## Artifacts", ""]
    for key, value in paths.items():
        lines.append(f"- `{key}`: `{value}`")
    lines += ["", "### Per-action detail", "", "| Action | n | NoMove Acc | Set Acc | SafeOracle Acc | Set gain | Mean regret | P90 regret | Headroom | G3 rate | Top-5% rate |", "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|"]
    for name, value in sorted(action.items(), key=lambda item: item[1]["Set_accuracy"]):
        lines.append(f"| {name} | {value['n']} | {value['NoMove_accuracy']:.2%} | {value['Set_accuracy']:.2%} | {value['SafeOracle_accuracy']:.2%} | {value['Set_gain_vs_NoMove']:+.2%} | {value['regret']['mean']:.3f} | {value['regret']['p90']:.3f} | {value['positive_headroom']['aggregate_capture']:.2%} | {value['G3_high_regret_rate']:.2%} | {value['catastrophic_top5_rate']:.2%} |")
    lines += ["", "### Semantic-region detail", "", "| Region | n | NoMove Acc | Set Acc | SafeOracle Acc | Set gain | Mean regret | P90 regret | Headroom | Set stay | Safe stay | G3 rate | Top-5% rate |", "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|"]
    for name, value in sorted(summary["region"].items()):
        lines.append(f"| {name} | {value['n']} | {value['NoMove_accuracy']:.2%} | {value['Set_accuracy']:.2%} | {value['SafeOracle_accuracy']:.2%} | {value['Set_gain_vs_NoMove']:+.2%} | {value['regret']['mean']:.3f} | {value['regret']['p90']:.3f} | {value['positive_headroom']['aggregate_capture']:.2%} | {value['Set_stay_rate']:.2%} | {value['SafeOracle_stay_rate']:.2%} | {value['G3_high_regret_rate']:.2%} | {value['catastrophic_top5_rate']:.2%} |")
    ...
    lines += ["", "### Geometry detail", "", f"- CandidateOracle absolute azimuth mean/median (all Episodes): {geometry['candidate_oracle_geometry']['abs_azimuth_deg']['mean']:.2f}° / {geometry['candidate_oracle_geometry']['abs_azimuth_deg']['median']:.2f}°.", f"- SafeOracle move absolute azimuth mean/median: {geometry['safe_oracle_move_geometry']['abs_azimuth_deg']['mean']:.2f}° / {geometry['safe_oracle_move_geometry']['abs_azimuth_deg']['median']:.2f}°; model-selected move: {geometry['model_selected_move_geometry']['abs_azimuth_deg']['mean']:.2f}° / {geometry['model_selected_move_geometry']['abs_azimuth_deg']['median']:.2f}°.", f"- CandidateOracle geodesic mean/median: {geometry['candidate_oracle_geometry']['geodesic']['mean']:.3f} / {geometry['candidate_oracle_geometry']['geodesic']['median']:.3f} m.", f"- SafeOracle move geodesic mean/median: {geometry['safe_oracle_move_geometry']['geodesic']['mean']:.3f} / {geometry['safe_oracle_move_geometry']['geodesic']['median']:.3f} m; model-selected move: {geometry['model_selected_move_geometry']['geodesic']['mean']:.3f} / {geometry['model_selected_move_geometry']['geodesic']['median']:.3f} m.", f"- Radius direction counts (closer/same/farther): CandidateOracle={geometry['radius_direction']['candidate_oracle_all']}; SafeOracle move={geometry['radius_direction']['safe_oracle_move']}; model-selected move={geometry['radius_direction']['model_selected_move']}.", "", "| SafeOracle move geodesic bin | n | Mean regret | Headroom capture |", "|---|---:|---:|---:|"]
    for name, value in geometry["geodesic_bins_by_safe_oracle_move"].items():
