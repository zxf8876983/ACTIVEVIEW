...
def write_analysis(result: Mapping[str, Any], path: Path) -> None:
    ...
    lines.extend([
        ...
        f"Compared with the historical Old shared head, the newly retrained OldFair head changes Stay accuracy by {(old['Stay/current']['accuracy'] - old_hist['Stay/current']['accuracy']) * 100:+.3f}pp, legal-micro accuracy by {(old['Legal candidate micro']['accuracy'] - old_hist['Legal micro']['accuracy']) * 100:+.3f}pp, and Random accuracy by {(old['Random legal']['accuracy'] - old_hist['Random legal']['accuracy']) * 100:+.3f}pp. The >1pp shift indicates head-retraining sensitivity; OldFair↔Yaw8Fair remains a controlled paired comparison, but promotion should be interpreted as a matched-protocol signal rather than an unconditional historical replication.", "",
    ...
    lines.extend([
        f"**Promotion: {promotion}.** This decision uses only matched-head legal/Random gains, Macro-F1 behavior, and the largest key-class Random decline ({critical_drop:.3f}pp). Because the matched Old head differs from the historical head by more than 1pp, this promotion is a controlled paired-encoder result and should not be read as an unconditional absolute claim.",
