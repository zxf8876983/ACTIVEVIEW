...
from activeview.scripts.run_exp051_r1_closed_loop import (
    _candidate_order,
    _joint_choice,
    _load_sources,
    build_rgb_cache,
...
def run(data_root: Path, checkpoint: Path, device: torch.device) -> dict[str, Any]:
    ...
    wm = _load_wm_e(Path("/tmp/activeview_exp042r1_E/last.pth"), device); stgcn = _load_stgcn(data_root, device)
    ...
    labels: list[int] = []; moving_labels: list[int] = []; h1_terminal: list[int] = []; h2_terminal: list[int] = []; h1_fused: list[int] = []; h2_fused: list[int] = []; h1_moves: list[int] = []; h2_moves: list[int] = []
    ...
    for row in rows:
        ...
        if order:
            ordered_imag = cache["imagined_logp"][idx, np.asarray(order, dtype=np.int64)]; ordered_true = cache["true_logp"][idx, np.asarray(order, dtype=np.int64)]; h0_imag.append(ordered_imag); h0_true.append(ordered_true); h0_labels.append(label); moving_labels.append(label)
        ...
        h2_moves.append(h2_count); h2_terminal.append(int(np.argmax(archive["logp"][h2_current]))); h2_fused.append(int(np.argmax(np.mean(np.exp(archive["logp"][h2_visited]), axis=0))))
...
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--data-root", type=Path, required=True); parser.add_argument("--checkpoint", type=Path, required=True); parser.add_argument("--device", default="cuda:0"); args = parser.parse_args(); device = torch.device(args.device)
