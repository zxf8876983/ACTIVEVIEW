...
# The persisted Stage A summary predates the current 21-scene HM3D-train
# protocol and still contains this legacy minival directory.  It is retained
# for historical provenance but must not participate in current scene coverage.
LEGACY_SCENE_IDS = frozenset({"00800-TEEsavR23oF"})
...
def main() -> int:
    ...
    target_scene_ids = [
        str(item)
        for item in summary.get("target_scene_ids", [])
        if str(item) not in LEGACY_SCENE_IDS
    ]
