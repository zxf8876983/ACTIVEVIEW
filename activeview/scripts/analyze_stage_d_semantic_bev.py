...
from activeview.active_view.stage_d_predictability import ACTION_NAMES, neighbor_agreement, neighbor_entropy, oracle_action_index, oracle_margin, margin_bin_index
...
def _world_samples(
    ...
    local = np.stack(((grid_x - cx) / fx, (cy - grid_y) / fx, -np.ones_like(grid_x)), axis=-1); local /= np.linalg.norm(local, axis=-1, keepdims=True)
...
def analyze(*, cache_root: Path, stage_b_root: Path, spatial_cache: Path, v0_val_predictions: Path, exp027_result: Path, output: Path, runtime: Path, source_root: Path, motion_root: Path, scene_root: Path, config_path: Path, workers: int = WORKERS, train_limit: int | None = None, val_limit: int | None = None) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {"experiment_id": EXP_ID, "experiment_name": "observed_local_semantic_bev_sufficiency_audit", "status": "COMPLETED", "decision": "INCONCLUSIVE", "split": "val", "test_used": False, "training_performed": True, "perception_regenerated": False, "habitat_rendering_performed": True, "stgcn_retrained": False, "smoke_gate": {"status": "PASS"}, "eligible_episode_counts": {"train": len(train_rows), "val": len(val_rows)}, "unique_visited_observations": {"train": len(train_keys), "val": len(val_keys), "union": len(set(train_keys) | set(val_keys))}, "generation": generation, "bev": {"shape": [15, 80, 80], "dtype": "uint8", "pool_grid": [10, 10], "feature_dim": int(train_vectors.shape[1]), "train_cache_bytes": (runtime / "train_bev.npz").stat().st_size, "val_cache_bytes": (runtime / "val_bev.npz").stat().st_size}, "nearest_neighbor": {"metric": "cosine", "index_split": "train", "agreement": nn_agreement}, "neighborhood_entropy": {"three_way_mean": float(np.mean([item["three_way"] for item in entropy])), "three_way_median": float(np.median([item["three_way"] for item in entropy])), "three_way_p25": float(np.percentile([item["three_way"] for item in entropy], 25)), "three_way_p75": float(np.percentile([item["three_way"] for item in entropy], 75)), "three_way_p90": float(np.percentile([item["three_way"] for item in entropy], 90)), "binary_mean": float(np.mean([item["binary"] for item in entropy])), "binary_median": float(np.median([item["binary"] for item in entropy]))}, "probe": probe, "representation": {"base": "EXP028 legal normalized Stage-D + visited RGB spatial vectors", "bev_channels": list(BEV_CHANNELS), "dimension": int(train_vectors.shape[1]), "normalization_stats_split": "train", "future_candidate_rgb_used": False, "future_candidate_semantic_used": False, "future_candidate_depth_used": False, "future_candidate_skeleton_used": False, "full_unobserved_semantic_map_used": False, "true_utility_used_as_model_input": False, "val_used_for_normalization": False, "val_used_for_neighbor_index": False}, "provenance": {"source_commit": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=REPO_ROOT, text=True).strip(), "stage_d_train_features_sha256": file_sha256(Path(summary["feature_files"]["train"])), "stage_d_val_features_sha256": file_sha256(Path(summary["feature_files"]["val"])), "spatial_cache_summary_sha256": file_sha256(spatial_cache / "summary.json"), "exp027_result_sha256": file_sha256(exp027_result)}}
