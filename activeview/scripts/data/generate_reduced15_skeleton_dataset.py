#!/usr/bin/env python3
"""Generate 30-frame estimated skeleton tensors for reduced-15 manifests.

This entry point intentionally uses the same canonical generator as the
reduced-12 protocol and does not enable temporal segment candidate pooling.
"""
...
def main() -> None:
    ...
    default_root = data_root / "datasets" / "reduced15_babel_diversity_v1"
    ...
    generator = BabelCleanDatasetGenerator(
        ...
        temporal_candidate_pool=False,
    )
    default_splits = ("train", "val") if args.subset == "stgcn_development" else ("train", "val")
    ...
    logging.getLogger(__name__).info("Generated reduced15 %s: %s", args.subset, summaries)
