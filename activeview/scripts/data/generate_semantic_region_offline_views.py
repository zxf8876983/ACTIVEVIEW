...
from activeview.data.motion.babel_clean_dataset_generator import BabelCleanDatasetGenerator,_load_resampled_motion,apply_humanoid_pose,precompute_grounding_offsets,transform_camera_sequence_to_gravity
...
def _worker(args, tasks, manifest, out):
 ...
 try:
  for num,(placement_id,record) in enumerate(tasks,1):
   placement=next(x for x in manifest['placements_data'] if str(x.get('placement_id',x.get('region')))==placement_id); base=np.asarray(placement['position'],np.float32); scene_yaw_deg=float(placement.get('yaw_deg',0.0)); motion=_load_resampled_motion(record,args.target_frames); conv=gen.converter.convert(motion); joints=np.asarray(conv['pose_motion']['joints_array'],np.float32); roots=np.asarray(conv['pose_motion']['transform_array'],np.float32); offsets,_=precompute_grounding_offsets(human,joints,roots,scene_yaw_deg=scene_yaw_deg); all_skel=[]; all_conf=[]
   for start in range(0,len(placement['viewpoints']),4):
    ...
    for fi,(pose,rt) in enumerate(zip(joints,roots)):
     apply_humanoid_pose(human,pose,rt,base_position=base,scene_yaw_deg=scene_yaw_deg,floor_y=float(base[1]),grounding_offset=float(offsets[fi])); obs=sim.get_sensor_observations(list(range(len(views))))
...
def main():
 ...
 items.sort(key=lambda x:(x['placement_id'],x['record_id'])); summary={'version':'semantic-region-offline-v2','scene_id':args.scene_id,'records':len(records),'placements':len(cm['placements_data']),'views_per_record':32,'samples':len(items)*32,'target_frames':args.target_frames,'image_size':args.image_size,'workers':args.workers,'rgb_saved':False,'depth_saved':False,'skeleton_shape_per_record':[32,3,args.target_frames,17],'navigation_reference':'placement_position','dynamic_reachability_must_be_recomputed_from_current_robot_position':True,'rotation_reference':'exact_offline_render_state','sensor_height_m':1.1,'target_height_m':0.85,'per_record_navigation_fields':['scene_id','navmesh_path','placement_position','viewpoint_positions','viewpoint_snapped_positions','viewpoint_agent_positions','viewpoint_rotations_wxyz','viewpoint_is_navigable','viewpoint_is_reachable_from_placement','viewpoint_navigation_cost_m'],'items':items}; (args.output_dir/'manifest.json').write_text(json.dumps(summary,indent=2,ensure_ascii=False)); print(json.dumps({'records':len(records),'placements':len(cm['placements_data']),'views':len(items)*32}))
