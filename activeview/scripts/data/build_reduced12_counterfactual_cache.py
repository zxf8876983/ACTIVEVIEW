...
def build_split(data_root: Path, split: str, wm_path: Path, output: Path, device: torch.device, batch_size: int, workers: int) -> dict[str, Any]:
    ...
    dataset = LazyWorldModelContextDataset(rows, _sources(data_root, rows), use_belief=True, target_scope="all", cache_size=64)
    ...
    summary = {"split": split, "contexts": len(episode_ids), "shape_imagined": list(result["imagined_logp"].shape), "wm_checkpoint": str(wm_path.resolve()), "use_rgb": False, "test_used": False}
