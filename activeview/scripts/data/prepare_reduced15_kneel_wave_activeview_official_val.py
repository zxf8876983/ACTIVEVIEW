...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=data_root / "datasets" / "reduced15_kneel_wave_babel_diversity_v1" / "activeview_official_val",
    )
    ...
    records, _ = collect_reduced12_records(
        ...
        labels=REDUCED15_KNEEL_WAVE_LABELS, record_prefix="reduced15kw",
    )
    selected = select_diverse_records(
        records, cap_per_class=args.cap, seed=args.seed + 1,
        labels=REDUCED15_KNEEL_WAVE_LABELS,
    )
    label_mapping = {label: i for i, label in enumerate(REDUCED15_KNEEL_WAVE_LABELS)}
    # Keep manifest order identical to the parallel generator's canonical
    # metadata/data order (record_id sort), so labels index the tensor rows.
    rows = _with_split(selected, "val", label_mapping)
    rows.sort(key=lambda item: str(item["record_id"]))
    args.output_root.mkdir(parents=True, exist_ok=True)
    (args.output_root / "label_mapping.json").write_text(
        json.dumps(label_mapping, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    (args.output_root / "val.json").write_text(
        json.dumps(rows, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    summary = {
        "protocol": "complete Official Val cap-100 active-sensing manifest",
        "categories": list(REDUCED15_KNEEL_WAVE_LABELS),
        "records": len(rows), "cap_per_class": args.cap, "seed": args.seed + 1,
        "source_split": "official_val", "test_used": False,
    }
