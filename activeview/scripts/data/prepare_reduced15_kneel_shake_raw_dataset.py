#!/usr/bin/env python3
"""Prepare raw-train/raw-val manifests for the wave-to-shake protocol."""
...
def main() -> None:
    ...
    parser.add_argument("--output-root", type=Path, default=data_root / "datasets" / "reduced15_kneel_shake_babel_diversity_v1")
    ...
    train_raw, train_excluded = collect_reduced12_records(
        ...
        labels=REDUCED15_KNEEL_SHAKE_LABELS, record_prefix="reduced15ks",
    )
    ...
    label_mapping = {label: i for i, label in enumerate(REDUCED15_KNEEL_SHAKE_LABELS)}
    ...
    stgcn_train, stgcn_val = _split_records(
        train_selected, (0.9, 0.1), args.seed, labels=REDUCED15_KNEEL_SHAKE_LABELS,
    )
    active_train, active_val, active_test = _split_records(
        val_selected, (0.6, 0.2, 0.2), args.seed + 1, labels=REDUCED15_KNEEL_SHAKE_LABELS,
    )
    ...
    summary = {
        "protocol": "reduced15 kneel + shake diversity-aware BABEL protocol",
        "categories": list(REDUCED15_KNEEL_SHAKE_LABELS),
        "label_mapping": label_mapping,
        "seed": args.seed,
        "caps": {"official_train_per_class": args.train_cap, "official_val_per_class": args.val_cap},
        "raw_train": {"train": len(stgcn_train), "val": len(stgcn_val), "diversity": {"train": _diversity(stgcn_train), "val": _diversity(stgcn_val)}},
        "raw_val": {"official_val": len(val_selected), "train": len(active_train), "val": len(active_val), "test": len(active_test), "diversity": {"official_val": _diversity(val_selected), "train": _diversity(active_train), "val": _diversity(active_val), "test": _diversity(active_test)}},
        ...
        "test_used": False,
        "source_files": {"official_train": str((args.babel_dir / "train.json").resolve()), "official_val": str((args.babel_dir / "val.json").resolve()), "amass_index": str(args.amass_index.resolve())},
    }
