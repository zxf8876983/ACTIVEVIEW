...
def _true_joint_examples(rows: Sequence[Mapping[str, Any]], cache: Mapping[str, np.ndarray], orders: Mapping[str, Sequence[int]], v1_by_episode: Mapping[str, int | None]) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    for row in rows:
        ...
        if if_none: continue
        i = indices[str(row["episode_id"])]; candidates = [int(v) for v in orders[str(row["episode_id"])] if int(v) not in {int(row["s0_viewpoint_id"]), int(row["s1_viewpoint_id"]), int(v1)}]
        ...
        candidate_rows.extend(np.concatenate([cache["true_logp"][i, c], cache["candidate_descriptor"][i, c], [0.0]]) for c in candidates)
...
def _fit_true_joint(rows: Sequence[Mapping[str, Any]], cache: Mapping[str, np.ndarray], orders: Mapping[str, Sequence[int]], v1_by_episode: Mapping[str, int | None], device: torch.device, output: Path) -> tuple[_JointRevision, dict[str, Any]]:
    ...
    if output.exists():
        ...
        if contexts == 0:
            contexts = len(_true_joint_examples(rows, cache, orders, v1_by_episode)[0])
    ...
    arrays = _true_joint_examples(rows, cache, orders, v1_by_episode); dataset = _JointDataset(*arrays); loader = DataLoader(dataset, batch_size=512, shuffle=True); optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4); history: list[float] = []
    ...
    output.parent.mkdir(parents=True, exist_ok=True); torch.save({"model_state_dict": model.state_dict(), "epoch": 20, "seed": SEED, "input": "true_future_recognition"}, output)
...
def run(data_root: Path, device: torch.device) -> dict[str, Any]:
    ...
    checkpoint = data_root / "experiments/stage_d/EXP054_h2_ceiling_decomposition/runtime/true_future_matched_joint_revision.pth"; model, train_info = _fit_true_joint(train_rows, train_cache, train_orders, train_v1, device, checkpoint)
...
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--data-root", type=Path, default=get_data_root()); parser.add_argument("--device", default="cuda:0"); args = parser.parse_args(); _seed(); device = torch.device(args.device)
