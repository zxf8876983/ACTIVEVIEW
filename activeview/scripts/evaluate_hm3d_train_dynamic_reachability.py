#!/usr/bin/env python3
"""Evaluate active-view policies with dynamic Habitat reachability.

The offline files contain one skeleton tensor for every action and candidate
viewpoint.  They also contain a placement-referenced navigation result, but
that result is not sufficient for a sequential robot trajectory.  This
evaluator therefore loads each scene navmesh once, samples one deterministic
initial candidate per scene/region, and recomputes paths from that current
robot position to all candidate viewpoints before applying the policies.

Only geometry is used for reachability.  ``Oracle`` is an explicit hindsight
upper bound that may inspect the frozen ST-GCN predictions after the dynamic
