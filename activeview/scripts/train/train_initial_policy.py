...
from activeview.data.preprocessing.policy_data import HardRecordAwareSampler, EpisodeFeatureDataset, RecordBalancedSampler, collate_episode_batch, load_feature_statistics
from activeview.methods.active_view.policy import evaluate_predictions, load_stage_b_lookup, predict_dataset
from activeview.methods.active_view.training import move_stay_loss, policy_loss
...
def train_model(*, feature_root: Path, stage_b_root: Path, output_dir: Path, model_type: str, device_name: str, batch_size: int, episodes_per_record: int, max_epochs: int, patience: int, lr: float, weight_decay: float, lambda_reg: float, lambda_rank: float, tau: float, lambda_gap: float = 0.0, tau_gap: float = 1.0, max_gap_weight: float = 10.0, lambda_move: float = 1.0, seed: int, sampler_type: str = "record_balanced", record_difficulty_path: Path | None = None, hard_episodes_per_record: int = 32, normal_episodes_per_record: int = 12) -> Dict[str, Any]:
    _seed(seed)
    ...
    difficulty_sha256 = None
    if sampler_type == "record_balanced":
        sampler = RecordBalancedSampler(train_set.rows, episodes_per_record=episodes_per_record, seed=seed)
        sampler_summary: Dict[str, Any] = {"mode": "record_balanced", "episodes_per_record_per_epoch": episodes_per_record}
    elif sampler_type == "hard_record":
        if record_difficulty_path is None:
            raise ValueError("record_difficulty_path is required for hard_record sampling")
        difficulty = json.loads(record_difficulty_path.read_text(encoding="utf-8"))
        hard_ids = [str(item["record_id"]) for item in difficulty.get("hard_records", [])]
        sampler = HardRecordAwareSampler(
            train_set.rows,
            hard_ids,
            hard_episodes_per_record=hard_episodes_per_record,
            normal_episodes_per_record=normal_episodes_per_record,
            seed=seed,
        )
        difficulty_sha256 = file_sha256(record_difficulty_path)
        sampler_summary = {
            "mode": "hard_record_aware",
            "hard_record_count": len(hard_ids),
            "normal_record_count": len(sampler.groups) - len(hard_ids),
            "hard_episodes_per_record_per_epoch": hard_episodes_per_record,
            "normal_episodes_per_record_per_epoch": normal_episodes_per_record,
            "difficulty_metric": "mean_safe_oracle_regret",
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        model.train(); loss_sums = {"total": 0.0, "regression": 0.0, "ranking": 0.0, "gap_ranking": 0.0, "move": 0.0}; steps = 0
        for batch in train_loader:
            ...
            target_utility = batch["utility_targets"].to(device)
            if model_type == "move_stay_set_ranker":
                move_logits = model.move_logits(batch["current_feature"].to(device))
                move_target = torch.tensor(
                    [
                        float(not bool(train_stage_b_lookup[str(episode_id)]["oracle"]["safe_oracle_stays"]))
                        for episode_id in batch["episode_id"]
                    ],
                    dtype=torch.float32,
                    device=device,
                )
                losses = move_stay_loss(
                    move_logits,
                    move_target,
                    predicted,
                    target_utility,
            ...
            steps += 1
        val_rows = predict_dataset(model, val_loader, stage_b_lookup, device, model_type=model_type)
        val_metrics = evaluate_predictions(val_rows, categories)
        val_score = float(val_metrics["recognition"]["StageC"]["macro_f1"])
        scheduler.step(val_score)
        history.append({"epoch": epoch, "train_loss": loss_sums["total"] / max(steps, 1), "train_regression_loss": loss_sums["regression"] / max(steps, 1), "train_ranking_loss": loss_sums["ranking"] / max(steps, 1), "train_gap_ranking_loss": loss_sums["gap_ranking"] / max(steps, 1), "train_move_loss": loss_sums["move"] / max(steps, 1), "val_metrics": val_metrics, "lr": optimizer.param_groups[0]["lr"]})
...
def main() -> None:
    ...
    parser.add_argument("--lr", type=float, default=1e-3); parser.add_argument("--weight-decay", type=float, default=1e-4); parser.add_argument("--lambda-reg", type=float, default=1.0); parser.add_argument("--lambda-rank", type=float, default=1.0); parser.add_argument("--tau", type=float, default=0.5); parser.add_argument("--lambda-gap", type=float, default=0.0); parser.add_argument("--tau-gap", type=float, default=1.0); parser.add_argument("--max-gap-weight", type=float, default=10.0); parser.add_argument("--lambda-move", type=float, default=1.0); parser.add_argument("--seed", type=int, default=42)
    args = parser.parse_args()
    result = train_model(feature_root=args.feature_root, stage_b_root=args.stage_b_root, output_dir=args.output_dir, model_type=args.model_type, device_name=args.device, batch_size=args.batch_size, episodes_per_record=args.episodes_per_record, max_epochs=args.max_epochs, patience=args.patience, lr=args.lr, weight_decay=args.weight_decay, lambda_reg=args.lambda_reg, lambda_rank=args.lambda_rank, tau=args.tau, lambda_gap=args.lambda_gap, tau_gap=args.tau_gap, max_gap_weight=args.max_gap_weight, lambda_move=args.lambda_move, seed=args.seed, sampler_type=args.sampler_type, record_difficulty_path=args.record_difficulty, hard_episodes_per_record=args.hard_episodes_per_record, normal_episodes_per_record=args.normal_episodes_per_record)
