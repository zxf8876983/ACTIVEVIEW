#!/usr/bin/env python3
"""Train the formal reduced12 Multi-positive Joint Revision on Train only."""
...
def _orders(data_root: Path, rows: Sequence[Mapping[str, Any]]) -> dict[str, list[int]]:
    ...
    pair, azimuth = load_pairwise_and_azimuths(data_root, rows, source, pair_root=data_root / "datasets/policy_reduced12_eight_placement_v1/pairwise_viewpoint_geodesic")
    return {str(row["episode_id"]): candidate_order(row, int(row["s1_viewpoint_id"]), {int(row["s0_viewpoint_id"]), int(row["s1_viewpoint_id"])}, pair[(str(row["scene_id"]), str(row["region"]))], azimuth[(str(row["scene_id"]), str(row["region"]))]) for row in rows}
...
def _examples(rows: Sequence[Mapping[str, Any]], cache: Mapping[str, np.ndarray], orders: Mapping[str, Sequence[int]]) -> tuple[tuple[np.ndarray, ...], dict[str, Any]]:
    ...
    currents: list[np.ndarray] = []; candidates_all: list[np.ndarray] = []; masks: list[np.ndarray] = []; fallbacks: list[int] = []; positives: list[np.ndarray] = []; labels: list[int] = []
    multi = single = none = 0; positive_counts: list[int] = []; max_actions = VIEW_COUNT - 1
    for row in rows:
        i = index[str(row["episode_id"])]
        candidates = [int(v) for v in orders[str(row["episode_id"])]]
        current_logp = np.asarray(cache["current_logp_s1"][i], dtype=np.float32); probs0 = np.exp(cache["current_logp_s0"][i]); probs1 = np.exp(current_logp)
        ...
        if positive_count:
            positive_counts.append(positive_count); multi += int(positive_count > 1); single += int(positive_count == 1); fallback = int(np.flatnonzero(action_correct)[0])
        else:
            none += 1; scores = [float(current_logp[int(row["label_id"])])] + [float(cache["true_logp"][i, c, int(row["label_id"])]) for c in candidates]; fallback = int(np.argmax(scores))
        ...
        currents.append(current); candidates_all.append(padded); masks.append(mask); fallbacks.append(fallback); positives.append(positive); labels.append(int(row["label_id"]))
