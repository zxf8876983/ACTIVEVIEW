...
def train_model(*, feature_root: Path, stage_b_root: Path, output_dir: Path, model_type: str, device_name: str, batch_size: int, episodes_per_record: int, max_epochs: int, patience: int, lr: float, weight_decay: float, lambda_reg: float, lambda_rank: float, tau: float, lambda_gap: float = 0.0, tau_gap: float = 1.0, max_gap_weight: float = 10.0, seed: int, sampler_type: str = "record_balanced", record_difficulty_path: Path | None = None, hard_episodes_per_record: int = 32, normal_episodes_per_record: int = 12) -> Dict[str, Any]:
    _seed(seed)
