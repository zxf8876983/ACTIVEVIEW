...
from activeview.data.preprocessing.policy_features import current_state_features, frozen_current_features
...
def rows(data_root: Path, split: str) -> list[dict[str, Any]]:
    """Load frozen Stage-D feature rows for non-Test analysis splits."""
...
def episode_sources(data_root: Path, split: str) -> dict[tuple[str, str, str], str]:
    """Map canonical context keys to archived skeleton paths."""
...
def load_stgcn(data_root: Path, device: torch.device) -> STGCN:
    ...
    model, _ = load_checkpoint(Path(summary["stgcn_checkpoint"]), len(mapping), str(device))
...
def load_observation_archive(
    source_path: Path, cache: Mapping[str, np.ndarray], context: tuple[str, str, str],
) -> dict[str, np.ndarray]:
    ...
    for feature in feature_rows:
        ...
        for candidate_id in remaining:
            ...
            geometry = second_step_geometry(
                ...
                relative_azimuth_deg=wrap_relative_azimuth(azimuths[candidate_id], azimuths[p1_id]),
            )
