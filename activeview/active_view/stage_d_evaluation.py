...
from activeview.active_view.stage_d_policy import order_candidates, second_step_decision, trajectory_cost
...
def build_single_step_oracles(stage_b_rows: Sequence[Mapping[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    """Materialize the Stage B CandidateOracle and SafeOracle baselines."""
    ...
    for record in stage_b_rows:
        ...
        candidate_item = by_id[candidate_id]
        candidate.append(_selected_row(record, candidate_id, moves=1, cost=float(candidate_item["geodesic_distance_m"])))
        if bool(oracle["safe_oracle_stays"]):
            ...
            safe_item = by_id[safe_id]
            safe.append(_selected_row(record, safe_id, moves=1, cost=float(safe_item["geodesic_distance_m"])))
