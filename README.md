...
v5.0 (MVP 5.0)   -> 【当前活跃】真实 Habitat Humanoid、RGB-D 渲染与 5 分类遮挡闭环
      ↓
未来版本 (v6/v7+) -> 继续完成 EA-AVS 端到端基本功能闭环（具体目标由后续规范定义）
```
...
> **提示**：
> - 历史版本目录（v1–v4）保持只读。完整科学闭环跑通前暂不重构为单一 `src/`。
> - 潜在后续方向包括：从真实 RGB-D 获取人体状态、姿态估计（Pose Estimation）、不确定性建模以及下游动作识别增益评价。这些仅作为候选研究方向，不预先绑定到特定版本号。
...
- **`AGENTS.md`**：所有 AI Agent 的统一第一入口、阅读顺序与科研行为守则。
