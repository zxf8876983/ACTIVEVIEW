...
class MultiSceneViewpointDatasetGenerator:
    ...
    def __init__(
        ...
        self.pipeline = HabitatPerceptionPipeline(data_root=self.data_root)
        ...
        for act_id, act_name in enumerate(ACTION_CLASSES):
            ...
            for inst_idx in range(num_instances_for_act):
                ...
                for vp in feasible_viewpoints:
                    ...
                    # 1. 调用真实环境感知流水线
                    cand_obs = self.pipeline.observe(
                        scene_id=scene_id,
                        human_state=human_placement,
                        robot_viewpoint=vp_dict,
                        base_motion_seq=base_skel,
                    )
                    assert cand_obs["skeleton_source"] == "estimated"

                    est_skel = cand_obs["skeleton"]
                    canon_skel = self.canonical_aligner.align(est_skel)
                    prediction = self.classifier.predict_sequence(canon_skel, is_normalized=True, apply_canonical=False)

                    entropy_val = float(prediction.entropy)
                    norm_entropy = float(np.clip(entropy_val / math.log(max(len(ACTION_CLASSES), 2)), 0.0, 1.0))
                    ...
                    mean_pose_conf = float(cand_obs["confidence"])
                    occ_res_dict = {
                        "occlusion_ratio": cand_obs["occlusion_ratio"],
                        "visible_joint_ratio": cand_obs["visible_ratio"],
