...
## Task
ACTIVEVIEW v7.0 Development — Humanoid-driven Active Perception Simulation Environment (v7.0 final polish completed)

## Objective
严格根据最新提交的 `EA_AVS_MVP70_Code_Generation_Document.md` 建立解耦、严谨、面向动作驱动的拟人化主动感知仿真环境，并完成最终打磨与验收：
1. **科研实验基础链路闭环全量通过**：
   ```text
...
2. **已完成终期打磨清单 (`ea_avs_mvp_v7/`)**：
   - `human/action_metrics.py`: 多维动力学动作真实性评价模块 (`ActionMotionMetrics` 包含 `height_change`, `pelvis_velocity`, `joint_motion_energy`, `torso_angle_change`, `orientation_change`, `dynamic_motion`)，可显著区分静态站立与动态跌倒；
   - `scripts/run_v7_demo.py`: 统一科研演示入口，分离 `robot_pose` (`position`, `rotation`) 与 `camera_pose` (`extrinsic`, `intrinsic`)，输出包含每帧 16 关键点 GT 与 `motion_metrics` 的标准化 `metadata.json`，并自动调用生成 `v7_demo.mp4`；
   ...
   - `tests/`: 组织为 `unit/`, `integration/`, `smoke/` 三层结构，22 项纯 Python 单元/集成测试全量 PASS。
...
- [x] 独立视频转换工具: `python -m ea_avs_mvp_v7.scripts.create_video --input data/ActiveView/visualizations/v7_demo/rgb --output data/ActiveView/visualizations/v7_demo/v7_demo.mp4` (PASS)
