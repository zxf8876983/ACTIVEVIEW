...
## 1. Current Stage
- **项目阶段**：**v8.2 — Local Active View Planning Final Baseline**（已彻底收尾与冻结，作为后续 v9 动作感知主动视角选择的坚实基线）。
- **历史基线**：
...
- **架构组织**：处于版本化递进过渡期（`v1 -> v2 -> v3 -> v4 -> v5 -> v6 -> v7 -> v8`），各版本保留独立目录与设计文档。

---

## 2. Active Development Version
- **Version**: v8.2 (8.2.0 — Local Active View Planning Final Baseline)
- **Title**: Local Active View Planning Final Baseline
...
## 4. v8.2 Research Objectives & Boundaries
...
2. **Robot 局部到位 (Robot in Human Vicinity)**：假设移动机器人已导航至人体所在局部空间周边区域 ($1.5\text{m} \sim 3.0\text{m}$)；
...
4. **View Quality Evaluation & Loss Penalty**: 依据透明科学评价公式 $Q(v) = w_1 \cdot \text{vis} + w_2 \cdot \text{cov} - w_3 \cdot \text{dist} - w_4 \cdot \text{loss}$ 对有效视点打分并排序 (`ViewQualityEvaluator`)；
5. **Baseline Strategies & Best Viewpoint Rendering**: 支持 `Random View`, `Nearest View`, `Geometry Best View` 三种选择策略 (`baseline_strategies.py`)，渲染生成高质量 `best_view_rgb.png`，并输出结构化统计 `view_selection_report.json`, `candidate_statistics.json`, `candidate_views.json` 与 `best_view.json`。

### 明确排除在 v8 之外的非目标 (Explicitly NOT in v8):
- ❌ 不实现 人体全局搜索与环境探索 (No Human Search / No Global Exploration)
- ❌ 不实现 Active View Selection 复杂动作感知决策 (NBV / 动作感知自适应网络，留待 v9)
