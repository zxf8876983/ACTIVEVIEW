...
职责：
    ...
    3. 基于感知置信度判定感知不确定性掩码 (occluded_mask / uncertainty_mask)；
...
class EstimatedSkeleton3D:
    """估计的 3D 人体骨架与感知质量结构体 (默认 COCO-17 原生拓扑)。"""
    joint_format: str                     # "COCO17"
    joints_3d_cam: np.ndarray             # 形状 (17, 3) 相机系坐标 (米)
    joints_3d_world: np.ndarray           # 形状 (17, 3) 世界系坐标 (米)
    joints_2d: np.ndarray                 # 形状 (17, 2) 图像系像素坐标
    confidence: np.ndarray                # 形状 (17,) 感知复合置信度 [0.0, 1.0]
    occluded_mask: np.ndarray             # 形状 (17,) bool (True=遮挡/感知不确定)
    part_confidence: Dict[str, float]     # 6 大部位平均感知置信度
    joints_3d_normalized: Optional[np.ndarray] = None  # 形状 (17, 3) 根节点归一化与尺度归一化坐标
...
class SkeletonConverter:
    ...
    def convert_and_fuse(
        ...
        occluded_mask = conf_composite < self.uncertainty_conf_thresh
        ...
        return EstimatedSkeleton3D(
            joint_format="COCO17",
            joints_3d_cam=joints_cam.copy(),
            joints_3d_world=joints_world.copy(),
            joints_2d=kpts_2d.copy(),
            confidence=conf_composite.astype(np.float32),
