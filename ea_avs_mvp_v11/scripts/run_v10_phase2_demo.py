...
from ea_avs_mvp_v11.core.paths import get_data_root, get_repo_root, get_v10_dataset_root
from ea_avs_mvp_v11.core.types import V10Sample
from ea_avs_mvp_v11.dataset.perception_dataset import V10PerceptionPipeline
from ea_avs_mvp_v11.perception.coordinate_validator import CoordinateValidator
from ea_avs_mvp_v11.perception.rgbd_skeleton_extractor import RGBDSkeletonExtractor
from ea_avs_mvp_v11.perception.skeleton_adapter import MediaPipe33ToCOCO17Adapter, MediaPipe33ToNTU25Adapter
...
from ea_avs_mvp_v11.visualization.skeleton_visualizer import SkeletonVisualizer
...
def run_phase2_demo():
    ...
    demo_dir = repo_root / "ea_avs_mvp_v11" / "examples" / "v10_phase2_demo"
...
> **Date**: 2026-08-22  
> **Target Version**: `ea_avs_mvp_v11` (Phase 2: RGB-D Skeleton Extractor -> Estimated 3D Skeleton)
...
| 核心模块 | 对应代码 | 验收状态 | 说明 |
|---|---|---|---|
| **成熟 RGB-D 骨架提取器** | `ea_avs_mvp_v11/perception/rgbd_skeleton_extractor.py` | **PASS** | 封装 `RGBDSkeletonExtractor` (MediaPipe 33 关键点 + 根节点深度几何对齐) |
| **3D 骨架空间归一化** | `ea_avs_mvp_v11/perception/skeleton_normalizer.py` | **PASS** | 根节点去中心化 + 躯干尺度归一化 (**严格禁止相机朝向旋转归一化**) |
| **坐标健康度校验器** | `ea_avs_mvp_v11/perception/coordinate_validator.py` | **PASS** | 物理深度范围、人体尺度、头部/脚踝相对上下位姿合理性检查 |
| **拓扑适配器接口** | `ea_avs_mvp_v11/perception/skeleton_adapter.py` | **PASS** | 提供 `MediaPipe33ToCOCO17Adapter` 与 `MediaPipe33ToNTU25Adapter` 接口 |
| **感知数据集持久化** | `ea_avs_mvp_v11/dataset/perception_dataset.py` | **PASS** | 保存 `skeleton_raw/`, `skeleton_normalized/`, `confidence/`, `metadata/` |
| **自动化单样本检查 CLI** | `tools/v10/check_sample.py` | **PASS** | 支持按 ID、按动作或随机挑选生成 4 面板高分辨率诊断图 |
| **多模态可视化套件** | `ea_avs_mvp_v11/visualization/skeleton_visualizer.py` | **PASS** | 严格基于官方拓扑渲染骨骼连线 |
