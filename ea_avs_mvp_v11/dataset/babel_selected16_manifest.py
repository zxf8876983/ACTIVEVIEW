"""Build the selected 16-class RGB/Habitat ST-GCN Train/Val manifests.

The official classes are taken from the previously audited BABEL-150
single-label records and capped at 400 Train / 100 Val per class.  ``lie`` and
``stumble`` are auxiliary safety labels (they are not part of the official-150
mapping) and retain every eligible record after the same length and
ambiguity filters.  This module only writes metadata; RGB perception is
performed by :mod:`babel_clean_dataset_generator` afterwards.
"""
...
from ea_avs_mvp_v11.dataset.babel_official150_true_skeleton import (
    cap_records,
    collect_official_records,
    load_official_categories,
)
from ea_avs_mvp_v11.dataset.babel_segment_utils import (
    _deduplicate,
    _filter_short_intervals,
    _iter_auxiliary_records,
    _remove_ambiguous_intervals,
)
from ea_avs_mvp_v11.dataset.babel_source_utils import _source_lookup
