...
功能：
    ...
    从 v2.0 迁移，CandidateView 新增了 v3.0 的评分字段。

注意：
    采样不考虑人体朝向评分（朝向评分在 predictive_evaluator 中计算）。
"""
...
class CandidateView:
    ...
    v3.0 的 pred_score 和 true_score 新增字段：
        - S_action_part_pred, S_orient_pred, S_action_part_true, S_orient_true
        - relative_view_angle
    """
    candidate_id: int
    position: np.ndarray
