...
# EXP007 — Val-only entry point; run after human approval.
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"
DATA_ROOT="${ACTIVEVIEW_DATA_ROOT:-${REPO_ROOT}/../../data/ActiveView}"
EXP_NAME="EXP007_candidate_relations"
...
(cd "${REPO_ROOT}" && python -m activeview.scripts.evaluate_stage_c_val \
  ...
  --experiment-id EXP007 > "${OUT}/result.json")
