...
# EXP002 — prepared only; run after human approval. Train and Val only.
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"
DATA_ROOT="${ACTIVEVIEW_DATA_ROOT:-${REPO_ROOT}/../../data/ActiveView}"
EXP_NAME="EXP002_hard_record_sampling"
OUT="${DATA_ROOT}/experiments/stage_c_v1/${EXP_NAME}"
DIFFICULTY="${REPO_ROOT}/experiments/stage_c_v1/${EXP_NAME}/train_record_difficulty.json"
...
(cd "${REPO_ROOT}" && python -m activeview.scripts.train_stage_c \
  ...
  --hard-episodes-per-record 32 \
  --normal-episodes-per-record 12 \
  --batch-size 128 \
  --max-epochs 100 \
  --patience 10 \
  --lr 0.001 \
...
(cd "${REPO_ROOT}" && python -m activeview.scripts.evaluate_stage_c_val \
  ...
  --experiment-id EXP002 > "${OUT}/result.json")
