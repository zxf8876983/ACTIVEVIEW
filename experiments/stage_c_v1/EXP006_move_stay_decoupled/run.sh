...
# EXP006 — Val-only entry point; run after human approval.
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"
DATA_ROOT="${ACTIVEVIEW_DATA_ROOT:-${REPO_ROOT}/../../data/ActiveView}"
EXP_NAME="EXP006_move_stay_decoupled"
...
(cd "${REPO_ROOT}" && python -m activeview.scripts.train_stage_c \
  --model-type move_stay_set_ranker \
...
(cd "${REPO_ROOT}" && python -m activeview.scripts.evaluate_stage_c_val \
  ...
  --checkpoint "${OUT}/checkpoints/move_stay_set_ranker_best.pth" \
  --output-dir "${OUT}/runtime" \
  --baseline "${SCRIPT_DIR}/baseline.json" \
  --experiment-id EXP006 > "${OUT}/result.json")
