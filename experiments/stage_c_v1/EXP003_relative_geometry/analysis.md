...
regret (target ≤ 1.3779728). EXP003 reached a 2.094% reduction. This record
does not assign the final keep/reject decision; it is left for user review.
...
`runtime/val_analysis_full.json` was generated by the unchanged generic failure
analysis code. Its geometry section has zero rows for EXP003 because the helper
currently accepts only 11-D arrays, while EXP003 rows contain 16-D geometry.
The reported geometry values above are an independent calculation from the
serialized cache; model metrics and all upstream accepted artifacts are
unchanged.
...
INCONCLUSIVE — final retain/reject decision is reserved for user scientific
review against the preregistered criterion.

### Next

1. User decides whether the sub-5% Val improvement justifies retaining EXP003.
2. If retained, repair the analysis helper's 16-D compatibility in a separate
   tooling change before relying on its geometry block in later reports.
