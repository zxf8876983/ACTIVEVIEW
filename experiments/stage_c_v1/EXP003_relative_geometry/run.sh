...
# EXP003 — prepared only; run after human approval. Train and Val only.
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../../.." && pwd)"
DATA_ROOT="${ACTIVEVIEW_DATA_ROOT:-${REPO_ROOT}/../../data/ActiveView}"
EXP_NAME="EXP003_relative_geometry"
...
(cd "${REPO_ROOT}" && python -m activeview.scripts.build_stage_c_relative_geometry_cache \
...
(cd "${REPO_ROOT}" && python -m activeview.scripts.evaluate_stage_c_val \
  ...
  --experiment-id EXP003 > "${OUT}/result.json")
