...
# EXP010 — Train-to-Val only; execute after human approval.
...
OUT="${DATA_ROOT}/experiments/stage_c_v2/EXP010_skeleton_policy_transformer"
BASELINE="${SCRIPT_DIR}/baseline.json"
CHECKPOINT="${OUT}/checkpoints/skeleton_policy_transformer_best.pth"
...
(cd "${REPO_ROOT}" && python -m activeview.scripts.train_stage_c_v2 \
  --model-type skeleton_policy_transformer --feature-root "${FEATURE_ROOT}" \
  --stage-b-root "${DATA_ROOT}/datasets/policy_v11_5/stage_b" \
  --output-dir "${OUT}/checkpoints" --batch-size 64 --episodes-per-record 16 \
...
(cd "${REPO_ROOT}" && python -m activeview.scripts.evaluate_stage_c_v2_val \
  ...
  --baseline "${BASELINE}" --output-dir "${OUT}/runtime" --experiment-id EXP010 \
  > "${OUT}/result.json")
