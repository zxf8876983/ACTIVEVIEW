...
# EXP008 — Train-to-Val only; execute after human approval.
...
(cd "${REPO_ROOT}" && python -m activeview.scripts.train_stage_c_v2 \
  --model-type joint_aware_set_ranker --feature-root "${FEATURE_ROOT}" \
...
(cd "${REPO_ROOT}" && python -m activeview.scripts.evaluate_stage_c_v2_val \
  ...
  --baseline "${BASELINE}" --output-dir "${OUT}/runtime" --experiment-id EXP008 \
  > "${OUT}/result.json")
