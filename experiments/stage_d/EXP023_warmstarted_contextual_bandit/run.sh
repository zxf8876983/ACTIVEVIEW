...
# EXP023: Train Phase A/B on Train, then evaluate once on Val.
...
EXP021_RESULT="${REPO_ROOT}/experiments/stage_d/EXP021_contextual_bandit/result.json"
OUT="${DATA_ROOT}/experiments/stage_d/EXP023_warmstarted_contextual_bandit"
...
test -f "${EXP021_RESULT}"
test -f "${V0_PREDICTIONS}"
mkdir -p "${OUT}"

(cd "${REPO_ROOT}" && python -m activeview.scripts.train_stage_d_warmstarted_bandit \
  --cache-root "${CACHE_ROOT}" \
  --stage-b-root "${STAGE_B_ROOT}" \
  --v0-predictions "${V0_PREDICTIONS}" \
  --exp021-result "${EXP021_RESULT}" \
