...
# EXP024: Train on Train and evaluate once on Val with visited RGB context.
...
RGB_RUNTIME_ROOT="${ACTIVEVIEW_RGB_RUNTIME_ROOT:-/home/zxf/MG08/robot/ActiveView}"
PYTHON_BIN="${ACTIVEVIEW_PYTHON:-python}"

"${PYTHON_BIN}" -m activeview.scripts.train_stage_d_rgb_context \
  ...
  --rgb-root "${RGB_ROOT}" \
  --embedding-cache "${RGB_RUNTIME_ROOT}/features/dinov2_vitb14_global" \
  --output "${SCRIPT_DIR}/result.json" \
  --runtime-dir "${RGB_RUNTIME_ROOT}/experiments/stage_d/EXP024_dinov2_rgb_context" \
  --seed 42 \
  --device cuda:0
