...
`lie`、`stumble` 是为老人安全感知保留的辅助标签；`fall` 当前不作为独立类。类别顺序来自 `ea_avs_mvp_v11/dataset/babel_selected16_manifest.py`，不得从旧数据集推断。
...
```bash
python ea_avs_mvp_v11/scripts/generate_semantic_region_offline_views.py --workers 4 --device cuda:0
```
...
python ea_avs_mvp_v11/scripts/evaluate_hm3d_train_dynamic_reachability.py --device cuda:0
python ea_avs_mvp_v11/scripts/evaluate_hm3d_train_random_initializations.py --device cuda:0
python ea_avs_mvp_v11/scripts/evaluate_hm3d_train_grid_initializations.py --device cuda:0
```
